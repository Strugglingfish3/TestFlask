...
from flask import Flask, request, render_template, send_from_directory, redirect, request, jsonify
...
def upload_files():
    ...
    # return jsonify({'progress': 100, 'filename': filename})
    return redirect('/gallery')

@app.route('/gallery')
def gallery():
    images = [f for f in os.listdir(UPLOAD_FOLDER) if f.endswith(('.jpg', '.png', '.gif'))]
